! NAGARCH with four noise distributions: Normal, Student-t, skewed-t, 2-component
! normal mixture (NM2).  Branched from xfit_garch_dist_returns.f90.
!
! NM2 parameterisation (3 free params after mean=0, variance=1 constraints):
...
!                                  sigma1 = sqrt(s*V/lambda)
!                                  sigma2 = sqrt((1-s)*V/(1-lambda))
! Starting values p5=p6=p7=0 give lambda=0.5, delta=0 → N(0,1).
...
! ── Main program ─────────────────────────────────────────────────────────────
program xnagarch_mix
! Fit NAGARCH with Normal, Student-t, skewed-t, and 2-component normal mixture.
...
integer,  parameter :: n_dist       = 4          ! Normal, t, skew-t, NM2
...
real(dp) :: d1s(n_dist), d2s(n_dist), d3s(n_dist)   ! distribution display params
real(dp) :: mu1s(n_dist), mu2s(n_dist)              ! NM2 component means (0 for others)
...
!          d1(8|8X), d2(8|8X), d3(8|8X), mu1(8|8X), mu2(8|8X),
!          vol%(9), logL(12), AIC(12), BIC(12), iter(6), skew(8), ekurt(8), 3×rank(6)
character(len=*), parameter :: hdr = &
    "(A9,A12,A8,A9,A9,A8,A8,A8,A8,A8,A9,A12,A12,A12,A6,A8,A8,A6,A6,A6)"
character(len=*), parameter :: fmt_n  = &
    "(A9,ES12.3,F8.4,F9.4,F9.4,8X,8X,8X,8X,8X,F9.2,F12.1,F12.1,F12.1,I6,F8.3,F8.3,3I6)"
character(len=*), parameter :: fmt_1p = &
    "(A9,ES12.3,F8.4,F9.4,F9.4,F8.3,8X,8X,8X,8X,F9.2,F12.1,F12.1,F12.1,I6,F8.3,F8.3,3I6)"
character(len=*), parameter :: fmt_2p = &
    "(A9,ES12.3,F8.4,F9.4,F9.4,F8.3,F8.3,8X,8X,8X,F9.2,F12.1,F12.1,F12.1,I6,F8.3,F8.3,3I6)"
character(len=*), parameter :: fmt_3p = &
    "(A9,ES12.3,F8.4,F9.4,F9.4,F8.3,F8.3,F8.3,F8.4,F8.4,F9.2,F12.1,F12.1,F12.1,I6,F8.3,F8.3,3I6)"
...
do icol = 1, ncols
    ...
    write(*, hdr) "Dist", "omega", "alpha", "theta", "beta", &
        "d1", "d2", "d3", "mu1", "mu2", "vol_ann%", "logL", "AIC", "BIC", &
    ...
    d1s(id) = 0.0_dp;  d2s(id) = 0.0_dp;  d3s(id) = 0.0_dp
    ...
    logl = -nobs*fopt;  aic = 2.0_dp*np - 2.0_dp*logl;  bic = np*log(real(nobs,dp)) - 2.0_dp*logl
    omegas(id) = omega;  alphas(id) = alpha;  betas(id) = beta;  thetas(id) = theta
    d1s(id) = nu;  d2s(id) = 0.0_dp;  d3s(id) = 0.0_dp
    mu1s(id) = 0.0_dp;  mu2s(id) = 0.0_dp
    vol_anns(id) = vol_ann;  logls(id) = logl;  aics(id) = aic;  bics(id) = bic
    niters(id) = niter;  conv(id) = converged
    call flex_skew_kurt(p, np, skews(id), kurts(id))
    ...
    logl = -nobs*fopt;  aic = 2.0_dp*np - 2.0_dp*logl;  bic = np*log(real(nobs,dp)) - 2.0_dp*logl
    omegas(id) = omega;  alphas(id) = alpha;  betas(id) = beta;  thetas(id) = theta
    d1s(id) = nu;  d2s(id) = gamma_sk;  d3s(id) = 0.0_dp
    mu1s(id) = 0.0_dp;  mu2s(id) = 0.0_dp
    vol_anns(id) = vol_ann;  logls(id) = logl;  aics(id) = aic;  bics(id) = bic
    niters(id) = niter;  conv(id) = converged
    call flex_skew_kurt(p, np, skews(id), kurts(id))
    ...
    logl = -nobs*fopt;  aic = 2.0_dp*np - 2.0_dp*logl;  bic = np*log(real(nobs,dp)) - 2.0_dp*logl
    omegas(id) = omega;  alphas(id) = alpha;  betas(id) = beta;  thetas(id) = theta
    d1s(id) = lam;  d2s(id) = sig1;  d3s(id) = sig2
    ...
    do id = 1, n_dist
        ...
        case (3)   ! skew-t: d1=nu, d2=gamma
            write(*, fmt_2p) trim(dist_names(id)), &
                ...
                vol_anns(id), logls(id), aics(id), bics(id), niters(id), &
                skews(id), kurts(id), rank_logl(id), rank_aic(id), rank_bic(id)
        case (4)   ! NM2: d1=lambda, d2=sig1, d3=sig2, mu1, mu2
...
write(*, '(A)') "        NM2 has 3 free dist params vs 2 for skewed-t"

call cpu_time(t_end)
write(*, '(/,A,F0.3,A)') "elapsed time: ", t_end - t_start, " s"

end program xnagarch_mix
