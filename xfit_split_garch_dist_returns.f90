...
! distributions to adjusted OHLC prices.
!
! Usage:
!   xfit_split_garch_dist_returns.exe [ohlc_prices_file] [output_csv]
!
...
! independent draws from the same selected innovation distribution.

module split_garch_dist_returns_mod
    ...
    use garch_split_fit_dist_mod, only: split_garch_dist_fit_result_t, fit_split_garch_dist_model, &
...
contains
    ...
    nobs = nprices - 1
    allocate(ret_cc(nobs), ret_co(nobs), ret_oc(nobs), vol(nobs))

    call print_price_sample_info(trim(prices_file), dates, nassets)
    write(*,'(A)') "Split CO/OC likelihood: h_co = co_frac*h_daily, h_oc = (1-co_frac)*h_daily; news impact uses CC returns."
    if (write_csv) then
    ...
    do iasset = 1, nassets
        ...
        do imod = 1, n_model
            do idist = 1, n_dist
                ...
                call fit_split_garch_dist_model(trim(models(imod)), trim(dists(idist)), ret_cc, ret_co, ret_oc, &
                                                max_iter, gtol, rows(nfit))
    ...
    end subroutine run_split_garch_dist_returns
    ...
    subroutine print_selection_summary(aic_wins, bic_wins)
        ...
        write(*,'(/,A)') "Split CO/OC AIC/BIC win counts"
        write(*,'(A)') repeat("-", 48)
...
end module split_garch_dist_returns_mod

program xfit_split_garch_dist_returns
    use split_garch_dist_returns_mod, only: run_split_garch_dist_returns
    implicit none

    call run_split_garch_dist_returns()
end program xfit_split_garch_dist_returns
