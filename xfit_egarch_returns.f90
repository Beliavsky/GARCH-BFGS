! Fit standard EGARCH(1,1)-Normal to the ETF return panel.
...
program xfit_egarch_returns
    ...
    use garch_fit_mod, only: fit_egarch, egarch_skew_kurt, egarch_persist
    implicit none
    ...
    write(*,'(A)') "Asset          omega    alpha    gamma     beta  persist  vol_ann%        logL         AIC         BIC  iter conv    skew   ekurt"
    write(*,'(A)') repeat("-", 128)
    ...
    do icol = 1, ncols
        ...
        call fit_egarch(ret, max_iter, gtol, fopt, params, niter, converged)
        ...
        write(*,'(A10,ES12.3,5F9.4,F12.2,F12.2,F12.2,I6,1X,L1,2F8.3)') &
            trim(col_names(icol)), params%omega, params%alpha, params%gamma, params%beta, persist, vol_ann, &
...
end program xfit_egarch_returns
