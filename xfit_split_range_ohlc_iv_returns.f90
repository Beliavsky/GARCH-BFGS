! Fit daily GARCH-family variance models using split CO returns plus high-low range.
...
program xfit_split_range_ohlc_iv_returns
    ...
    logical,  parameter :: use_range_only_oc_support = .true.
    ...
    real(dp), allocatable :: ret(:), ret_co(:), ret_oc(:), range_var(:), lo_oc(:), hi_oc(:)
    ...
    real(dp), allocatable :: active_cc(:), active_co(:), active_oc(:), active_range(:), active_lo(:), active_hi(:)
    ...
    ncols   = size(prices, 2)
    nobs    = nprices - 1
    allocate(ret(nobs), ret_co(nobs), ret_oc(nobs), range_var(nobs), lo_oc(nobs), hi_oc(nobs))
    allocate(vol_forecast(nobs), vol_forecasts(nobs,ncols,n_model))
    allocate(active_cc(nobs), active_co(nobs), active_oc(nobs), active_range(nobs), active_lo(nobs), active_hi(nobs))
    ...
    write(*,'(A)') "Split+range likelihood: CO Gaussian term, Parkinson range quasi-likelihood, and truncated OC Gaussian term."
    if (use_range_only_oc_support) then
        write(*,'(A)') "The OC density is conditioned on range-only support |OC| <= log(H/L); h_oc = (1-co_frac)*h_daily."
    else
        write(*,'(A)') "The OC density is conditioned on the observed high-low interval; h_oc = (1-co_frac)*h_daily."
    end if
    ...
    do icol = 1, ncols
        ...
        oc_mean = mean(ret_oc)
        ret_oc = ret_oc - oc_mean
        range_var = (log(high_prices(2:nprices,icol) / low_prices(2:nprices,icol)))**2 / (4.0_dp*log(2.0_dp))
        if (use_range_only_oc_support) then
            hi_oc = max(abs(log(high_prices(2:nprices,icol) / low_prices(2:nprices,icol))), abs(ret_oc)) + 1.0e-12_dp
            lo_oc = -hi_oc
        else
            lo_oc = log(low_prices(2:nprices,icol) / open_prices(2:nprices,icol)) - oc_mean
            hi_oc = log(high_prices(2:nprices,icol) / open_prices(2:nprices,icol)) - oc_mean
            lo_oc = min(lo_oc, ret_oc - 1.0e-12_dp)
            hi_oc = max(hi_oc, ret_oc + 1.0e-12_dp)
        end if

        active_cc = ret
        active_co = ret_co
        active_oc = ret_oc
        active_range = max(range_var, 1.0e-12_dp)
        active_lo = lo_oc
        active_hi = hi_oc
    ...
    real(dp) function split_value(p, np)
        ...
        do t = 1, size(active_cc)
            ...
            split_value = split_value + log_sqrt_2pi + 0.5_dp*(log(h_co) + active_co(t)**2/h_co) + &
                          truncated_normal_nll(active_oc(t), active_lo(t), active_hi(t), h_oc) + &
                          log(h_oc) + active_range(t)/h_oc
            h = next_variance(active_cc(t), h, params, active_model)
        end do
        split_value = split_value / real(size(active_cc), dp)
    end function split_value

    real(dp) function truncated_normal_nll(y, lo, hi, h)
        real(dp), intent(in) :: y, lo, hi, h
        real(dp) :: sig, a, b, z, prob

        if (h <= 0.0_dp .or. h /= h .or. y < lo .or. y > hi) then
            truncated_normal_nll = huge(1.0_dp)
            return
        end if
        sig = sqrt(max(h, 1.0e-12_dp))
        a = lo / sig
        b = hi / sig
        z = y / sig
        prob = normal_cdf(b) - normal_cdf(a)
        if (prob <= 1.0e-14_dp .or. prob /= prob) then
            truncated_normal_nll = huge(1.0_dp)
        else
            truncated_normal_nll = log_sqrt_2pi + log(sig) + 0.5_dp*z*z + log(prob)
        end if
    end function truncated_normal_nll

    real(dp) function normal_cdf(x)
        real(dp), intent(in) :: x
        normal_cdf = 0.5_dp * (1.0_dp + erf(x / sqrt2))
        normal_cdf = min(max(normal_cdf, 0.0_dp), 1.0_dp)
...
end program xfit_split_range_ohlc_iv_returns
