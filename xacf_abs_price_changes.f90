! Compute autocorrelations of absolute close-to-close price changes
! for several intraday sampling frequencies.

program xacf_abs_price_changes
    ...
    real(dp), allocatable :: abs_change(:)
    real(dp) :: acf(max_lag)
    ...
    print '(A)', "Autocorrelations of absolute close-to-close price changes"
    ...
    do ifreq = 1, size(freq_seconds)
        if (freq_seconds(ifreq) == source_seconds) then
            call close_abs_changes(regular_series, abs_change, nchange)
        else
            call resample_ohlcv_series(regular_series, sampled_series, source_seconds, freq_seconds(ifreq))
            call close_abs_changes(sampled_series, abs_change, nchange)
        end if
        call autocorr(abs_change(1:nchange), acf)
        print '(I8,1X,I10,10(1X,F8.4))', freq_seconds(ifreq), nchange, acf
        if (allocated(abs_change)) deallocate(abs_change)
    end do
    print '(A)', "------------------------------------------------------------------------------------------------------------"

contains

    ! Compute within-day absolute close-to-close price changes.
    subroutine close_abs_changes(series, abs_change, nchange)
        type(ohlcv_series_t), intent(in) :: series
        real(dp), allocatable, intent(out) :: abs_change(:)
        integer, intent(out) :: nchange
        integer :: i

        allocate(abs_change(max(series%nobs() - 1, 0)))
        nchange = 0
        do i = 2, series%nobs()
            if (yyyymmdd(series%timestamp(i)%date) /= yyyymmdd(series%timestamp(i - 1)%date)) cycle
            nchange = nchange + 1
            abs_change(nchange) = abs(series%close(i) - series%close(i - 1))
        end do
        if (nchange < max_lag + 2) error stop "close_abs_changes: not enough observations"
    end subroutine close_abs_changes
...
end program xacf_abs_price_changes
