...
program xacf_intraday_measures
    ...
    integer :: ifreq, nchange, nrange, ifile
    ...
    do ifile = 1, size(filenames)
        ...
        if (has_extension(filenames(ifile), ".bin")) then
            call read_ohlcv_tick_stream(filenames(ifile), tick_series)
            call convert_tick_to_ohlcv_series(tick_series, raw_series)
        else
            call read_intraday_prices_csv(filenames(ifile), raw_series)
        end if
        ...
        print '(A,A)', "Input file: ", trim(filenames(ifile))
    ...
    deallocate(acf_signed, acf_abs, acf_range, filenames)
